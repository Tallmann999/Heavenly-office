...
| core_divine_office | Сквозной Divine Office Flow | Steam-демо на 2 дня: 6 душ в первый день и 9 душ во второй | Проверяет главную игровую гипотезу | MVP | integration_strategy, content_souls, content_reincarnations, persistence, localization_service | premium | done | `docs/2026-06-07_steam-demo-gd-spec.md` |
| integration_strategy | Интеграция с первой итерацией | Сохранить первую итерацию отдельным режимом и сделать ясный выбор режима | Исключает конфликт двух flow | MVP | legacy_office_loop | premium | done | `docs/2026-06-04_divine-office-feature-roadmap_gd-spec.md` |
...
| main_office_experience | Главный экран кабинета | Очередь, окно души, досье, печати, порталы, рычаг, HUD; весы/зеркало как тизер полной версии | Формирует запоминаемый игровой стол и главный Steam-скриншот | MVP | integration_strategy, localization_service | premium | done | `docs/2026-06-07_steam-demo-gd-spec.md` |
| reincarnation_result_ui | Экран результата | Исходная душа, новая форма, причина, награды, CTA продолжения | Даёт понятный и смешной payoff | MVP | reincarnation_resolver, localization_service | premium | in_progress | `docs/2026-06-07_steam-demo-gd-spec.md` |
| content_souls | Контент дел душ | 15 душ в прохождении демо: 10 обычных, 3 редких, 1 эпик, 1 легенда; общий пул может быть 25 душ | Обеспечивает разнообразие решений | MVP | localization_service | premium | done | `docs/2026-06-04_divine-office-feature-roadmap_gd-spec.md` |
...
| reward_and_cards | Награды и открытие карт | Награды по редкости, монеты, очки судейства и карта-досье за идеальное решение | Создаёт кратко- и среднесрочную мотивацию | MVP | persistence, karma_evaluator, soul_card_collection | premium | in_progress | `docs/2026-06-04_divine-office-feature-roadmap_gd-spec.md` |
...
| soul_card_collection | Коллекция карт душ | Доступна сразу; полная сетка, фильтры, детали справа, прогресс, редкость, бонусы и новые уникальные карточки | Даёт долгосрочную цель и ценность редких душ | MVP | reward_and_cards, persistence | premium | planned | `docs/features/soul-card-collection.md` |
| world_state_day_summary | Итоги дня и состояние миров | Упрощённая карта веток галактики, рейтинг судьи, новые карточки, ресурсы и новые ветки | Показывает влияние игрока | MVP | world_impact, persistence | premium | planned | `docs/2026-06-04_divine-office-feature-roadmap_gd-spec.md` |
...
| office_upgrades | Улучшения канцелярии | Зеркало/пронзительный взгляд, весы и другие инструменты стола после демо | Формирует мета-прогрессию и помогает решать сложные души | P1 | reward_and_cards, world_state_day_summary | premium | planned | `docs/2026-06-04_divine-office-feature-roadmap_gd-spec.md` |
| assistants | Помощники канцелярии | Стартовый милосердный помощник и будущие персонажи с советами, сведениями, ветками и моральным уклоном | Добавляют стратегию, юмор и новые способы читать дела | MVP | soul_card_collection, judge_rating, office_upgrades | premium | planned | `docs/2026-06-04_divine-office-feature-roadmap_gd-spec.md` |
| demo_settings | Настройки демо | RU/EN, задел под 20 языков, звук, музыка, SFX, mute, fullscreen/windowed, resolution | Делает демо пригодным для Steam-аудитории | MVP | localization_service, persistence | premium | planned | `docs/2026-06-07_steam-demo-gd-spec.md` |
| demo_postgame | Продолжение после финала демо | После финала можно играть дальше с сильно сниженным шансом редких/уникальных душ | Даёт replay value без обесценивания полной версии | MVP | content_souls, reward_and_cards, persistence | premium | planned | `docs/2026-06-07_steam-demo-gd-spec.md` |
| full_game_progress_transfer | Перенос прогресса в полную версию | Сохранить карты и прогресс демо для покупки полной игры | Убирает страх потерять редкие карты | MVP | persistence, soul_card_collection | premium | planned | `docs/2026-06-07_steam-demo-gd-spec.md` |
...
- `world_state_day_summary` → `world_impact`, `judge_rating`, `persistence`, затем `office_upgrades`, `assistants` и `day_events`
...
- Закрытый стартовый путь `core_divine_office`: игра стартует через `MainOfficeExperience` и создаёт `DivineOfficeDemo` без Legacy-режима.
...
1. Реализовать барабан реинкарнаций после печати и определить правила качества решения.
2. Довести расследование души до состояния, где намерения и скрытая информация влияют на выбор.
3. Завершить результат, награды, сохранение и смену языка без потери состояния.
4. После стабильного цикла добавить коллекцию карт и итоги дня.

## Top-3 Рисковые Фичи

1. `soul_investigation`: это главное отличие продукта, но его точные правила не определены.
2. `return_case_queue`: возврат дел после Проверки/Апелляции требует ясных правил времени, новых сведений и завершения.
3. `reincarnation_wheel`: барабан должен стать наградным моментом, а не задержкой между решением и результатом.

## Предложенный Порядок Разработки

1. Принять решения по вопросам P0 из сводного GD-дока.
2. Зафиксировать правила расследования, оценки и реинкарнации.
3. Довести 5 тестовых душ до полноценного контента и закрыть сохранение/локализацию.
4. Проверить гипотезу плейтестом: интересно ли изучать дело, ставить печать и видеть результат.
5. Только после этого расширять вертикальный срез коллекцией, мирами, улучшениями и событиями.
